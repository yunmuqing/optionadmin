$(document).ready(function () {
    ...
    if ($("input[name='id']").length == 1){
        var dom = $($("input[name='applies_type']:checked").attr('data-class'));
        if (dom.length){
    ...
    $("input,textarea,select").bind('input propertychange', function() {
        ...
        console.log(form_data,$('#defaultForm').serialize());
                ...
                },
                {
                    ...
                    handler: function(modal) {
                        if (!$(this).hasClass('disabled')){
                            ...
                            var res = [];
                            modal.find('.variants-checkbox:checked').each(function () {
                                res.push($(this).val());
                            })
            ...
            pushHtml += `
                    <li class="list-group-item">
                        <div class="custom-control custom-checkbox">
                          <input type="checkbox" ${variant.indexOf(item['id']) == -1 ? '' : 'checked data-checked="1"' }  name="variants-checkbox" value="${item['id']}" class="custom-control-input variants-checkbox" id="variants-checkbox-${item['id']}" data-image="${item['image']}" data-sid="${item['sid']}" data-product-id="${that.parents('.list-group-item').attr('data-id')}">
    ...
    function showHtml(res,modal,that) {
            ...
            data.forEach(function (item) {
                ...
                if (that.attr('data-type') == 'products'){
                    if (item['variants'].length != variant_value.length){
                ...
                if (that.attr('data-type') == 'products'){
                    ...
                    variants.forEach(function (variant) {
                        pushHtml += `
                            <li class="list-group-item">
                                <div class="custom-control custom-checkbox">
                                  <input type="checkbox" ${variant_value.indexOf(variant['id']) == -1 ? '' : 'checked data-checked="1"' } data-parent="${item['id']}" name="variant-${item['id']}" value="${variant['id']}" class="custom-control-input shopify-variant-checkbox" id="product-${variant['id']}" data-image="${item['image']}" data-title="${item['title']}">
    ...
    $('#defaultForm').bootstrapValidator({
                    ...
                    callback:{
                        ...
                        callback:function(value,validator){
                            if ($(".range_min").length>0) {
                              if (!isInteger($(".range_min").val()/$(".range_commissions").val())) {
                                   return false
                                }else{
                                    return true
                                }  
        ...
        $.ajax({
            ...
            dataType: "json",
            //请求地址
            url : "/api/commissionsredeem/edit",
            //数据，json字符串
