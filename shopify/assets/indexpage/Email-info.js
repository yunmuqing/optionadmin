$(document).ready(function () {
    ...
    $('.restore').on('click',function () {
        $(this).fireModal({
            title: 'Restore email',
            body: 'Are you sure you want to restore the email? Your previous configuration will be overwritten after you restore the email!',
            center: true,
            initShow: true,
            ...
            buttons: [
                ...
                {
                    text: 'Restore',
                    class: 'btn btn-primary btn-shadow btn-restore',
                    handler: function(modal) {
                        that = $('.btn-restore')
                        that.addClass('btn-progress');
                        $.ymqajax({
                            url: " /api/email/reset",
                            data: {id:$('.restore').data('id')},
    ...
    $('.send').on('click',function () {
        $(this).fireModal({
            ...
            body: `
                ...
                <label class="col-12" style="font-size: 18px;">email:</label>
                <div class="col-sm-12">
                <input type="text" class="form-control email-input" name="email" value="" required="">
                </div>
                </div>
            `,
            center: true,
            ...
            buttons: [
                ...
                {
                    text: 'Send',
                    class: 'btn btn-primary btn-shadow btn-send',
                    handler: function(modal) {
                        that = $('.btn-send')
                        that.addClass('btn-progress');
                        $.ymqajax({
                            url: " /api/email/send",
                            data: {type:$('.send').data('type'),email:$('.email-input').val()},
                            success: function (res) {
                                iziToast.show({
        ...
        $.ajax({
            ...
            success : function(result) {
                if (result.code == 1) {
                    ...
                    if(status == 1){
                        that.attr('data-status',0).addClass('btn-secondary').removeClass('btn-primary').html('Disable').prev().removeClass('off').html('Default email is live');
                    }else {
                        that.attr('data-status',1).removeClass('btn-secondary').addClass('btn-primary').html('Enable').prev().addClass('off').html('Default email is disabled');
                    }
