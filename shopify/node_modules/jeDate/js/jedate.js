    ...
    jet.extend(jeDatePick.prototype,{
        ...
        eachDays  : function (yd,md,ds,idx) {
            ...
            var regExpDate = function (date,cls,ymyear,ymqmonth) {
                ...
                //console.log(jeDate.getLunar(ymqdateobg).nW)
                ...
                if(endval.length > 0 && endval[0]!=""){
                    ...
                    if(/\%/g.test(endval[0])){
                        ...
                        //限定周
                        var enval = endval[0].split(",")||[];
                        enval.forEach(function (item,index) {
                            if (item.indexOf("ymq") != -1) {
                                ymq_week = item.replace(/ymq/g,'').split('-');
                                delete enval[index];
                            }
                        })
                        if (enval.length > 0) {
                            ymq_endval = enval.join(",");
                        }else{
                            ymq_endval = "";
                        }
